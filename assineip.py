...
NETWORK = '224.0.0.22'  # Adjust to your network range
...
def ip_exists_in_netbox(ip):
    ...
    if response.status_code == 200:
        data = response.json()
        return data.get('count', 0) > 0  # Returns True if IP already exists
    else:
        print(f"Failed to check if IP exists: {response.status_code}")
        return False
